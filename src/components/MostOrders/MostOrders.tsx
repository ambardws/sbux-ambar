        ...
        {orders?.map((order, index) => (
          <div
            ...
            className={`card bg-base-100 shadow-md p-4 min-w-[170px] relative inline-block mt-[10px] ml-[10px] ${
              orders.length - 1 === index ? "!mr-5" : ""
            }`}
          >
            <div className="stack-top left font-semibold text-sm">{order.discount}%</div>
              ...
              {order.discount !== 0 ? (
                <div className="flex space-x-2">
                  <p className="mt-2 text-xs font-light text-[#BEC9E0] line-through">
                    {numberFormat({
                      num: Number(order.price),
                      opt: { style: "currency" },
                    })}
                  </p>
                  <p className="mt-2 text-xs font-bold">
                    {numberFormat({
                      num: Number(order.price - order.price * 0.1),
                      opt: { style: "currency" },
                    })}
                  </p>
                </div>
